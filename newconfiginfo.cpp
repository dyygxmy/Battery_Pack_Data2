...
void Newconfiginfo::initui()
{
    ...
    ui->showStation->setText(configIniRead->value("baseinfo/showStation").toString());
...
void Newconfiginfo::baseInfoIsChange()
{
    ...
    if (controllerIp_01 != configIniRead->value("baseinfo/ControllerIp_1").toString()||
            ...
            ui->showStation->text() != configIniRead->value("baseinfo/showStation").toString() )
    {
        e3 = new QGraphicsOpacityEffect(this);
        e3->setOpacity(0.5);
        ui->label_100->setGraphicsEffect(e3);
...
void Newconfiginfo::receiveBaseinfoSave(bool statetmp)
{
    ...
    if(statetmp)
    {
        ...
        configIniRead->setValue("baseinfo/showStation",ui->showStation->text());
...
void Newconfiginfo::on_pushButton_62_clicked()
{
    ...
    else
    {
        ...
        ui->lineEdit_TightResetOnline->setText(configIniRead->value("baseinfo/TightResetOnline").toString());//拧紧返修
...
//节拍统计翻页查询处理
void Newconfiginfo::pagechange1()
{
    qDebug()<<"pagechange1";
    thepages =QString::number((thepage-1)*10);
    aff = "select ID,packSN,packType,timeStart,timeEnd,diffTime,employeeID,packStatus from PackOutputStatus " + condition +" ORDER BY ID DESC limit "+thepages +", 10;";;
    qDebug()<< "aff1="<< aff;
    model1->setQuery(aff,db);
    int i;
    for(i=0;i<10;i++)
    {
        ui->tableView_2->setRowHeight(i,43);
    }
}
...
{
    qDebug()<<"on_pushButton_search_clicked";
    //    ui->tabWidget->setCurrentIndex(0);
    if(!db.isOpen())
    {
        if(!db.open())
    ...
    query = new QSqlQuery(db);
    if(ui->tabWidget->currentIndex()==0)//数据
    {
        ...
        else if(ui->stackedWidget_dateSelect->currentIndex() == 1)
        {
            if(ui->dateEdit_max->text()<ui->dateEdit_min->text())
            {
                ...
                ui->dateEdit_max->setStyleSheet("QDateTimeEdit {font: 14pt \"Arial\";border-width:1px; border-style:solid; border-color:rgb(250, 0, 0); }QDateTimeEdit::drop-down { width:30px; border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::down-arrow {image: url(:/LCD_CS351/LCD_CS351/logo/arrow.png);}");
                ui->dateEdit_min->setStyleSheet("QDateTimeEdit {font: 14pt \"Arial\";border-width:1px; border-style:solid; border-color:rgb(250, 0, 0); }QDateTimeEdit::drop-down { width:30px; border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::down-arrow {image: url(:/LCD_CS351/LCD_CS351/logo/arrow.png);}");
                return;
            }
            else
            {
                ui->dateEdit_max->setStyleSheet("QDateTimeEdit {font: 14pt \"Arial\";border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::drop-down { width:30px; border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::down-arrow {image: url(:/LCD_CS351/LCD_CS351/logo/arrow.png);}");
                ui->dateEdit_min->setStyleSheet("QDateTimeEdit {font: 14pt \"Arial\";border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::drop-down { width:30px; border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::down-arrow {image: url(:/LCD_CS351/LCD_CS351/logo/arrow.png);}");
    ...
    else if(ui->tabWidget->currentIndex()==1)//趋势曲线
    {
        condition = "";
    ...
    else if(ui->tabWidget->currentIndex()==2)//节拍统计
    {
        QString iniCondition = " where timeStart != 'NULL' and timeEnd != 'NULL'  and timeStart is not null and timeEnd is not null and packStatus = '1' "
                " and TO_DAYS(timeStart) > TO_DAYS(\"2020-01-01 00:00:00\") and TO_DAYS(timeEnd) > TO_DAYS(\"2020-01-01 00:00:00\") ";
        if(ui->lineEdit_PACKSN->text().size() > 0){//有PACKSN就按PACKSN来查
            condition = iniCondition + " and packSN like '%" + ui->lineEdit_PACKSN->text()+"%' ";
        }else{//没有PACKSN的按时间来查
            if(ui->stackedWidget_dateSelect->currentIndex() == 0)//三天/一周/一月查询
            {
                int diffDays = QDateTime::fromString("2020-01-01 00:00:00","yyyy-MM-dd hh:mm:ss").daysTo(QDateTime::currentDateTime());//默认查到2020-01-01 00:00:00
                if(ui->pushButton_threeDays->isChecked())
                    diffDays = 3;
                else if(ui->pushButton_oneWeek->isChecked())
                    diffDays = 7;
                ...
                condition = iniCondition + " and TO_DAYS(timeEnd) > TO_DAYS(NOW())-" + QString::number(diffDays);
            }
            else if(ui->stackedWidget_dateSelect->currentIndex() == 1)//自定义时间查询
            {
                if(ui->dateEdit_max->text()<ui->dateEdit_min->text())//选择时间不合理
                {
                    ui->label_dateWrong->show();
                    ui->dateEdit_max->setStyleSheet("QDateTimeEdit {font: 14pt \"Arial\";border-width:1px; border-style:solid; border-color:rgb(250, 0, 0); }QDateTimeEdit::drop-down { width:30px; border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::down-arrow {image: url(:/LCD_CS351/LCD_CS351/logo/arrow.png);}");
                    ui->dateEdit_min->setStyleSheet("QDateTimeEdit {font: 14pt \"Arial\";border-width:1px; border-style:solid; border-color:rgb(250, 0, 0); }QDateTimeEdit::drop-down { width:30px; border-width:1px; border-style:solid; border-color:rgb(51, 153, 255); }QDateTimeEdit::down-arrow {image: url(:/LCD_CS351/LCD_CS351/logo/arrow.png);}");
                ...
                else
                {
                    ...
                    QString midTime = ui->dateEdit_min->text();
                    QString maxTime = ui->dateEdit_max->text();
                    condition = iniCondition + " and timeEnd >= '"+midTime+"' and timeEnd <= '"+maxTime+"'";
                }
            }else{
                condition = iniCondition;
            }
        }
        affall = "SELECT timeStart,timeEnd FROM PackOutputStatus "+condition + " ORDER BY ID DESC;";
        qDebug()<< "affall1="<< affall;
        if(!query->exec(affall))//获取数据条数，方便翻页处理
            qDebug()<<query->lastError();
        else
        {
            int numRows = query->numRowsAffected();
            qDebug() << "row number1: " << numRows;
            if (numRows%10 != 0)
            {
                pages = numRows/10+1;
            }
            ...
            ui->label_M_2->setText(QString::number(pages));
            if (numRows==0)
            {
                ui->label_N_2->setText("0");
                thepage = 0;
            }
            else
            {
                thepage = 1;
                ui->label_N_2->setText(QString::number(thepage));
            }
            ...
            int sum_beat = 0;
            while(query->next()){
                QDateTime timeStart = QDateTime::fromString(query->value(0).toString(),"yyyy-MM-ddThh:mm:ss");//2020-04-08 12:30:46
                QDateTime timeEnd = QDateTime::fromString(query->value(1).toString(),"yyyy-MM-ddThh:mm:ss");
                sum_beat += timeStart.secsTo(timeEnd);
//                qDebug()<<"thisBeat:"<<timeStart<<timeEnd<<timeStart.secsTo(timeEnd)<<query->value(0).toString();
            }
            if(numRows > 0){
                ui->label_beat->setText("当前平均节拍：" + QString::number(sum_beat/numRows));
            }

            //查询前10条数据显示到界面tableView_2
            thepages = "0";
            aff = "select ID,packSN,packType,timeStart,timeEnd,diffTime,employeeID,packStatus from PackOutputStatus " + condition +" ORDER BY ID DESC limit "+thepages +", 10;";
            qDebug()<< "aff1="<< aff;
            model1->setQuery(aff);
...
void Newconfiginfo::on_pushButton_first_2_clicked()
{
    if(thepage == 0)
        thepage = 0;
    else
...
void Newconfiginfo::on_pushButton_last_2_clicked()
{
    thepage = pages;
    ui->label_N_2->setText(QString::number(thepage));
    pagechange1();
...
//节拍统计上一页
void Newconfiginfo::on_pushButton_previous_2_clicked()
{
    //left
    if(thepage == 1)
        thepage = 2;
...
//节拍统计下一页
void Newconfiginfo::on_pushButton_next_2_clicked()
{
    //right
    if(thepage == pages)
        thepage = pages-1;
...
void Newconfiginfo::connect_localMySQLPack()
{
    //打开本地数据库
    ...
    if(!db.open())
    {
        qDebug()<< "newconfig1" << db.lastError().text();
    }else
    {
        qDebug()<< "newconfig1 database ok";
    }
...
//    query_bound = QSqlQuery(db);
    ...
    model1->setQuery("select * from PackOutputStatus WHERE ID <0 and packStatus = '1'",db);
    model1->setHeaderData(0, Qt::Horizontal, tr("ID"));
    model1->setHeaderData(1, Qt::Horizontal, tr("PackSN"));
    model1->setHeaderData(2, Qt::Horizontal, tr("类型"));
    model1->setHeaderData(3, Qt::Horizontal, tr("开始时间"));
    model1->setHeaderData(4, Qt::Horizontal, tr("结束时间"));
    model1->setHeaderData(5, Qt::Horizontal, tr("当前节拍"));
    model1->setHeaderData(6, Qt::Horizontal, tr("员工号"));
    model1->setHeaderData(7, Qt::Horizontal, tr("状态"));

//    model1->setHeaderData(3, Qt::Horizontal, tr("扭矩"));
//    model1->setHeaderData(4, Qt::Horizontal, tr("角度"));


//    ui->tableView_2->horizontalHeader()->setResizeMode(QHeaderView::Fixed);
...
//    ui->tableView_2->setEditTriggers(QAbstractItemView::NoEditTriggers);//不可编辑
    ...
    ui->tableView_2->setShowGrid(false);//显示表格线

    ui->tableView_2->setColumnWidth(0,100);//第一列宽度
    ...
    ui->tableView_2->setColumnWidth(7,100);

    int i;
    for(i=0;i<10;i++)
    {
        ui->tableView_2->setRowHeight(i,43);//行宽
    }
    ui->tableView_2->horizontalHeader()->setStyleSheet("QHeaderView::section {background-color: rgb(51, 153, 255);"
                                                     "color: rgb(248, 248, 255);border: 0px; font:14pt}");
    ui->tableView_2->horizontalHeader()->setFixedHeight(51);
    ui->tableView_2->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft);

    ui->tableView_2->verticalHeader()->setVisible(false);   //隐藏列表头
    ui->tableView_2->horizontalHeader()->setVisible(true); //隐藏行表头
    ui->label_M_2->setText("0");
    ui->label_N_2->setText("0");

    thepages = "";


//    ui->tableView_2->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);//一直显示垂直滚动条
//    ui->tableView_2->verticalScrollBar()->setStyleSheet("QScrollBar:vertical{"//垂直滑块整体
...
//    ui->tableView_2->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOn);//一直显示水平滚动条
//    ui->tableView_2->horizontalScrollBar()->setStyleSheet("QScrollBar:horizontal{"//水平滑块整体
//    "background:#FFFFFF;"
//    "padding-top:3px;"
//    "padding-bottom:3px;"
//    "padding-left:20px;"
...
{
    historyclear();
    ui->stackedWidget_condition->setCurrentIndex(currentTab);//stackedWidget_condition与tabWidget的页面是对应的
...
//        ui->stackedWidget_condition->setCurrentIndex(2);
}
...
void Newconfiginfo::revResultFromSql(int avg)
{
    qDebug()<<"avg"<<avg;
//    ui->lineEdit_PACKSN->setText(QString::number(avg));
//    ui->pushButton_search_2->setText("查询:");
