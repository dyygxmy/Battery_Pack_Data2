...
void Newconfiginfo::receiveSaveState(bool statetmp)
{
    ...
    if(statetmp)
    {
        ...
        if(ui->radioButton_wireless->isChecked())
            configIniRead->setValue("baseinfo/connectNet","0");//无线
        else if(ui->radioButton_wire1->isChecked())
            configIniRead->setValue("baseinfo/connectNet","1");//有线1
        else if(ui->radioButton_wire2->isChecked())
            configIniRead->setValue("baseinfo/connectNet","2");//有线2
                ...
                ui->second->text().toInt() != second)
        {
            ...
            //qDebug()<<"str"<<str;
            system(QString("ps | grep -E \"date\" | grep -v \"grep\"|awk '{print $1}' | xargs kill -9").toLatin1().data());
