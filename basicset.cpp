    ...
    ui(new Ui::BasicSet)
{
   ...
   // this->setCursor(Qt::BlankCursor); //去掉光标
    ...
    ui->lineEdit_newcount->setText(QString::number(GRemaningCountSpearhead));
    ui->label_count->setText(QString::number(GRemaningCountSpearhead));
    GWarningCountSpearhead = QSettings("/config.ini", QSettings::IniFormat).value("baseinfo/warningCountSpearhead").toInt();
    ui->lineEdit_lowCount->setText(QString::number(GWarningCountSpearhead));
...
//更新批头次数
void BasicSet::on_pushButton_new_clicked()
{
    GRemaningCountSpearhead = ui->lineEdit_newcount->text().toInt();
    ui->label_count->setText(QString::number(GRemaningCountSpearhead));
    QSettings("/config.ini", QSettings::IniFormat).setValue("baseinfo/remaningCountSpearhead",GRemaningCountSpearhead);

    GWarningCountSpearhead = ui->lineEdit_lowCount->text().toInt();
