                ...
                if((unsigned char)revComBuf[i] == 0x0A)
                {
                    if(revComBuf.size() >= 88)
                    {
                        ...
                        if(airResultValidFlag)//air test
                        {
                            ...
                            airTestResult(QString(revComBuf));
                        }
                        revComBuf.clear();
                        revStartFlag = false;
                    }
...
// 120 STAT= 0 KAN= 1 OP= 0 PP= 1 DP=     0  A DT=   0.0 VL=   -1.62 PF= 3.480 PM= 2.521 A\r\n
//KAN 通道号 上面为1 下面为2
//PP 程序号
//VL 泄漏率（cm3/min）
//PM 测量压力 （bar）
...
{
    buf = buf.trimmed();//去掉首尾空字符
    buf.replace(QRegExp("[\\s]+"), " ");  //把所有的多余的空格转为一个空格
