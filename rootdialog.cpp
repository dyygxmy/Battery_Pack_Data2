...
//    connect(keyread,SIGNAL(sendKey(int)),this,SLOT(receiveKey(int)));
    ...
    fileDialog = new showPictureDialog;
}
...
//     emit sendWarningSpearhead();
    if(tmpKey == redKey || tmpKey == greenKey || tmpKey == blackKey || allowNoKey){
...
//    on_pushButton_5_clicked(); //测试注销双弹窗
}
...
{
    if(tmpKey == redKey || tmpKey == greenKey || allowNoKey){
        SaveWhat = "";
...
//图片及文件
void RootDialog::on_pushButton_code_2_clicked()
{
    if(tmpKey == redKey || tmpKey == greenKey || allowNoKey){
        if(isLogin || tmpKey ==greenKey || tmpKey == redKey)
        {
