...
void StepParameter::on_pushButton_save_clicked()
{
    ...
    if(!ui->groupBox->isHidden())
    {
        if(ui->stackedWidget->currentIndex()==0)//barcode set interface
        {
            ...
            singleStep["parameter"]=ui->lineEdit_scanTimes->text()+","+ui->comboBox_PDM->currentText()+","+ui->lineEdit_regexp->text()+","+","+ui->lineEdit_regexp_2->text()+","+ui->lineEdit_regexpCode->text()+","+ui->lineEdit_scanPass->text()+","+ui->lineEdit_dateDiff->text()+","+ui->lineEdit_VictionRegexp->text();
        }
