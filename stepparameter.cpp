...
void StepParameter::on_radioButton_completed_clicked()
{
    ...
    CompletedList<<tr("辊道托盘到位")<<tr("托盘到位匹配产品型号")<<tr("站完成请求PCL放行托盘")
               ...
               <<tr("模组入箱完成")<<tr("本次模组搬运完成")<<tr("手动/自动模式")<<tr("启动开班信号")
              <<tr("开班信号")<<tr("Pack1状态")<<tr("Pack2状态")<<tr("下线托盘型号")
             <<tr("涂胶数据绑定完成")<<tr("涂胶拧紧开班信号")<<tr("涂胶拧紧Pack1状态")<<tr("Pack1SN")<<tr("Pack2SN");
...
void StepParameter::on_radioButton_manual_clicked()
{
    ...
    ManualList<<tr("向服务器请求产品型号")<<tr("条码枪扫码")<<tr("拧紧")<<tr("泄漏")
             <<tr("等待I/O输入")<<tr("I/O输出")<<tr("跳转")<<tr("显示PDM")<<tr("手工站站完成")
            <<tr("操作PACK1")<<tr("操作PACK2")<<tr("辊道线数据绑定")<<tr("挂起");
...
void StepParameter::on_pushButton_save_clicked()
{
    ...
    if(!ui->groupBox->isHidden())
    {
        if(ui->stackedWidget->currentIndex()==0)//barcode set interface
        {
            ...
            singleStep["parameter"]=ui->lineEdit_scanTimes->text()+","+ui->comboBox_PDM->currentText()+","+ui->lineEdit_regexp->text()+","+ui->lineEdit_regexpCode->text();
        }
        ...
        else if(ui->stackedWidget->currentIndex()==2)//PDM show set interface
        {
            ...
            singleStep["parameter"]=ui->lineEdit_PDMShowTime->text()+","+ui->comboBox_PDM_2->currentText();
        }
...
bool StepParameter::judgment()
{
    ...
    if(!ui->groupBox->isHidden())
    {
        ...
        {
            if(ui->lineEdit_scanTimes->text().isEmpty())
                return false;
        }
        else if(ui->stackedWidget->currentIndex()==2)
        {
            if(ui->lineEdit_PDMShowTime->text().isEmpty())
                return false;
        }
        ...
        {
            if(ui->lineEdit_Signature->text().isEmpty())
                return false;
        }
        else if(ui->stackedWidget->currentIndex()==5)
        {
