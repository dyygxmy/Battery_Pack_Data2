...
void Interface::reReqPackStatus()
{
    ...
    {
        QVariantMap result = resultStruct.Result;
        bool Result = result["Result"].toBool();Result = true;
        if(!Result)
        {
            ErrorCode = result["Errorcode"].toString();
...
void Interface::Keybarcode(QString keyBarcode,QString PackSN ,QString time, int OrderSN)
{
   ...
   // QString url = "/upload/Keybarcode?";    //true
    ...
    if(resultStruct.isSuccess)
    {
        ...
        if(!Result)
        {
            emit sendKeyCodeInfo(Line_ID,OrderSN,GscanRegExpCode,keyBarcode,PackSN,Station);//没成功上传到接口，发到本地数据库存储
    ...
    {
        //没成功上传到接口，发到本地数据库存储
        emit sendKeyCodeInfo(Line_ID,OrderSN,GscanRegExpCode,keyBarcode,PackSN,Station);
        GIsInterfaceUpSuccess = false;
