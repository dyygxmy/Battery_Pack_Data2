...
//        QObject::connect(&taotong,SIGNAL(sendStopSignal(bool)),stepCtrl,SLOT(revStopButtonSignal(bool)),Qt::AutoConnection);
    ...
    QObject::connect(PLC1,SIGNAL(sendgetStationTimer(QString,int)),interface,SLOT(revgetStationTimer(QString,int)),Qt::AutoConnection);//request dutou tight status
    ...
    QObject::connect(interface,SIGNAL(sendGetStationTimer(QDateTime)),PLC1,SLOT(recgetStationTimer(QDateTime)),Qt::AutoConnection);//receive product type
