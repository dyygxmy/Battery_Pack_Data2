...
//    QTimer::singleShot(0,cardRead,SLOT(comInit()));
    ...
    */
    ...
    PLC1 = new ConnectPLC;
    PLC1->channel_Num = 1;
    QTimer::singleShot(0,PLC1,SLOT(PLCStart()));
    if(variable2 == "D")
    {
        sleep(1);
        PLC2 = new ConnectPLC;
        PLC2->channel_Num = 2;
        QTimer::singleShot(0,PLC2,SLOT(PLCStart()));
    }
...
//        QObject::connect(&taotong,SIGNAL(sendStopSignal(bool)),stepCtrl,SLOT(revStopButtonSignal(bool)),Qt::AutoConnection);
    ...
    QObject::connect(interface,SIGNAL(sendModulePack(bool)),PLC1,SLOT(revModulePack(bool)),Qt::AutoConnection);//receive ACK
    QObject::connect(newconfiginfo,SIGNAL(sendTrayCntinue(bool)),PLC1,SLOT(revModulePack(bool)),Qt::AutoConnection);//receive ACK
    QObject::connect(PLC1,SIGNAL(sendShowPush()),newconfiginfo,SLOT(onShowPushButon()),Qt::AutoConnection);

    QObject::connect(PLC1,SIGNAL(sendReqKeybarcode(QString, QString, QString, int)),interface,SLOT(Keybarcode(QString, QString ,QString,int)),Qt::AutoConnection);//key barcode upload
    ...
    if(variable2 == "D")
    {
        ...
        QObject::connect(interface,SIGNAL(sendModulePack(bool)),PLC2,SLOT(revModulePack(bool)),Qt::AutoConnection);//receive ACK
        QObject::connect(PLC2,SIGNAL(sendReqKeybarcode(QString, QString, QString, int)),interface,SLOT(Keybarcode(QString, QString ,QString,int)),Qt::AutoConnection);//key barcode upload
