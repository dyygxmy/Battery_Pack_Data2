...
#include "ui_stepeditbev.h"

stepeditbev::stepeditbev(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::stepeditbev)
{
    ui->setupUi(this);
    ...
    currentStep = parsejson->parseJsonbev();
    delete parsejson;
    if(currentStep.value("errorCode")!=1)
        initUI(currentStep);
}

void stepeditbev::initUI(QVariantMap Step)
{
    stepList.clear();
...
void stepeditbev::on_pushButton_return_clicked()
{
    QVariantMap Step;
...
void stepeditbev::newStepParameter()
{
...
void stepeditbev::on_pushButton_append_clicked()
{
    newStepParameter();
...
void stepeditbev::on_pushButton_insert_clicked()
{
...
void stepeditbev::on_pushButton_update_clicked()
{
    if(ui->tableWidget->currentRow()!= -1)
    {
        newStepParameter();
...
void stepeditbev::on_pushButton_remove_clicked()
{
...
void stepeditbev::on_radioButton_manual_clicked()
{
    ui->label_plcDB->hide();
    ui->label_plcIP->hide();
...
void stepeditbev::on_radioButton_auto_clicked()
{
    ui->label_plcDB->show();
    ui->label_plcIP->show();
...
void stepeditbev::on_pushButton_save_clicked()
{
    whichSave = "save";
    newSave();
}

void stepeditbev::saveStep(bool isSave)
{
    if(isSave)//确认保存
    {
        QVariantMap Step;
        ...
        if (ok)
        {
            ...
            if(stepJson.open(QIODevice::WriteOnly | QIODevice::Truncate))
            {
                ...
                qDebug()<<"stepbev.json write open fail";
        }
        else
...
void stepeditbev::receiveStep(QVariantMap singleMap)
{
    QTableWidgetItem *tableWidgetItem = new QTableWidgetItem(singleMap["singleStep"].toString());
    QTableWidgetItem *tableWidgetItem1 = new QTableWidgetItem(singleMap["parameter"].toString());
    //QTableWidgetItem *tableWidgetItem2 = new QTableWidgetItem(singleMap["parameter"].toString());
...
void stepeditbev::closeStepParameter()
{
    delete e3;
    ui->label_black->hide();
