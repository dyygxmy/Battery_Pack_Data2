...
{
    qDebug()<<"Ningbo ReInsertServerDbNingBo thread start";
...
void ReInsertServerDbNingBo::timer_insert()
{
    ...
    while(1)
    {
        ...
        if(WIFI_STATE)
        {
            ...
            int uploadmark = 0;
            QString msg="";
            //            qDebug() << "******** 1111111 ********" << db1.isOpen()<<db2.isOpen()<<db3.isOpen();

            {
                ...
                if(db2.isOpen() && QSqlDatabase::contains("ReinsertMySQL"))
                {
                    if(!query2.exec("SELECT RecordID, IDCode, ScrewID, Torque, Angle, Curve, TighteningStatus, TighteningTime,UploadMark,Cycle FROM TighteningDatas Where UploadMark =2 or UploadMark =4 order by RecordID limit 10"))
                    {
                    ...
                    query2.exec("SELECT RecordID, IDCode, ScrewID, Torque, Angle, Curve, TighteningStatus, TighteningTime,UploadMark,Cycle FROM TighteningDatas Where UploadMark =3 and Curve !='null' order by RecordID limit 5");
                    while(query2.next())
                    {
                        qDebug()<<"*****************come in 3*************************";
                        //              datasql = false;
